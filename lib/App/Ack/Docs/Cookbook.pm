...
C<< \b >> says word boundary here, letters one side and space or puctuation on the other,
...
C<ack -w pattern> will restrict the match to C<pattern> as a word, surrounded by whitespace or puntuation
...
at the begining of line B<OR> preceded by a word OR by a space (or tab etc), before the needed punc,
SWIM as C<(?:^|\b|\s)#+\s+(.*)>
which will find and capture a comment.
(Try it plain, with C<-o>, and with C<--output='$1'>, which has its own section below)

So don't look for C<ack -w '$identier'>, it won't match C<+$identifier> or C<func($identifier)> and won't even find it in column 1; do instead
...
Use C<ack>'s filetype inference to find things of C<--type=perltest>
...
=head2 TBD these are notes to be fleshed out TBD

=head2 TBD Mention dirty and/or git diff --name-only.

=head2 TBD  Search all the files that don't have foo and show their usage of bar. Think of a better example.
...
The C<< (?x: ) >>  says spaces don't count, are used for readibility.
...
=head2 Summarize the filetypes in your project
...
to annotate the URLs found in a file with their download weighti:
